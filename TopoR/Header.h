...
struct Header {                              // Раздел «Заголовок файла».
    struct Units {                           // Единицы измерений для всего файла.
        [[= XML::Attr("dist")]] dist dist{}; // Единицы измерения длины для всего файла. Значение по умолчанию – mm (миллиметр).
        [[= XML::Attr("time")]] time time{}; // Единицы измерения времени для всего файла. Значение по умолчанию – ps (пикосекунда).
    };
    ...
    [[= XML::Elem("Units")]] Units Units;       // Единицы измерения для всего файла.
};
} // namespace TopoR
