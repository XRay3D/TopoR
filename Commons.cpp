...
void Line::Shift(float x, float y) {
    if (_Dots.empty() ? false : _Dots.size() > 0) {
        for (std::int32_t i = 0; i < _Dots.size(); i++) {
            _Dots[i]->Shift(x, y);
        }
    }
}
...
void Polyline::Shift(float x, float y) {
    ...
    //        for (std::int32_t i = 0; i < _Segments.size(); i++) {
    //            (dynamic_cast<IBaseSegment*>(_Segments[i]))->Shift(x, y);
    //        }
    //    }
}
...
void Polyline::UnitsConvert(dist_ in_units, dist_ out_units) {
    ...
    //    if (_Segments.empty() ? false : _Segments.size() > 0) {
    //        for (std::int32_t i = 0; i < _Segments.size(); i++) {
    //            (dynamic_cast<IBaseSegment*>(_Segments[i]))->UnitsConvert(in_units, out_units);
    //        }
    //    }
}
...
void Detail::Shift(float x, float y) {
    //  FIXME  if ((dynamic_cast<IBaseFigure*>(_Figure)) != nullptr) {
    //        (dynamic_cast<IBaseFigure*>(_Figure))->Shift(x, y);
    //    }
}
...
void Detail::UnitsConvert(dist_ in_units, dist_ out_units) {
    ...
    // FIXME   if ((dynamic_cast<IBaseFigure*>(_Figure)) != nullptr) {
    //        (dynamic_cast<IBaseFigure*>(_Figure))->UnitsConvert(in_units, out_units);
    //    }
}
...
float Ut::UnitsConvert(double value, dist_ in_units, dist_ out_units) {
    double k;
    switch (in_units) {
    case dist_::mkm:
        k = 0.001;
